...
docker_temp_server_start() {
	...
	set -- "$@" -c max_wal_size=8192 -c listen_addresses='0.0.0.0' -p "${PGPORT:-5432}"
...
_main() {
	...
	if [ "$1" = 'postgres' ] && ! _pg_want_help "$@"; then
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
			...
			cat <<-'EOM'
				...
				FOO PostgreSQL Database directory appears to contain a database; Skipping initialization but running entry scripts
	...
	echo "Skipping start of psql daemon"
}
